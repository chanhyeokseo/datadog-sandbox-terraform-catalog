...
class ECSPresetManager:
    ...
    def _scan_local_presets(self) -> Dict[str, Dict]:
        presets = {}
        if not self.ecs_dir.exists():
            return presets
        for d in sorted(self.ecs_dir.iterdir()):
    ...
    def _scan_s3_presets(self, force: bool = False) -> Dict[str, Dict]:
        ...
        try:
            ...
            for name in sorted(preset_names):
                ...
                local_manifest = self.ecs_dir / name / "manifest.json"
    ...
    def get_preset_file(self, name: str, filename: str) -> Optional[str]:
        local_path = self.ecs_dir / name / filename
    ...
    def create_preset(self, name: str, description: str = "", preset_type: str = "aws-ecs",
    ...
    def clone_preset(self, source_name: str, target_name: str) -> bool:
        ...
        target_dir = self.ecs_dir / target_name
    ...
    def get_layout(self) -> List[Dict]:
        layout = None
        local_path = self.ecs_dir / "_layout.json"
    ...
    def initialize_local_cache(self) -> None:
        s3 = self._get_s3_manager()
        if not s3:
            logger.warning("S3 not available, skipping ECS preset cache initialization")
            self._cache_initialized = True
            return

        self.ecs_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            for key in keys:
                ...
                if rel.startswith("_"):
                    continue
                local_file = self.ecs_dir / rel
                local_file.parent.mkdir(parents=True, exist_ok=True)
                if s3.download_file(key, local_file):
                    downloaded += 1

            logger.info(f"ECS preset cache initialized: {downloaded} files downloaded from S3")
        except Exception as e:
            logger.warning(f"Failed to initialize ECS preset cache: {e}")

        self._cache_initialized = True

    def refresh_from_s3(self) -> None:
        logger.info("Refreshing ECS preset cache from S3")
        self._cache_initialized = False
        self.initialize_local_cache()

    def _deployments_path(self) -> Path:
        return self.ecs_dir / "_deployments.json"
