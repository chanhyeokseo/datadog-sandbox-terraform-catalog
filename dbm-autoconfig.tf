...
output "dbm_autoconfig_connection_info" {
  description = "Connection information for the auto-configured DBM setup"
  value       = <<-EOT
    
    ====== DBM Auto-Config Complete! ======
    
    EC2 Instance:
      SSH: ${module.dbm_autoconfig_ec2.ssh_command}
    
    RDS PostgreSQL:
      Host: ${module.dbm_autoconfig_rds.db_endpoint}
      Port: ${module.dbm_autoconfig_rds.db_port}
      Database: datadog
      Admin User: ${var.rds_username}
      Datadog User: datadog
    
    Datadog DBM:
      Status: Automatically configured
      Check: sudo datadog-agent status | grep -A 20 'postgres'
